...
import { fetchLiveFeedItems } from '@/lib/liveFeedCache';
...
const FEED_WIDE_QUERY = '(min-width: 1340px)';
...
function useDesktopFeedColumn(): boolean {
    const [ok, setOk] = useState(false);

    useEffect(() => {
        const mq = window.matchMedia(FEED_WIDE_QUERY);
        const sync = (): void => {
            setOk(mq.matches);
        };
...
export default function LiveFeed() {
    ...
    const desktop = useDesktopFeedColumn();
    ...
    useEffect(() => {
        ...
        fetchLiveFeedItems()
            .then((list) => {
                setItems(list.map(feedItemToEntry).slice(0, MAX_FEED_ITEMS));
            })
            .catch(() => {
    ...
    useEffect(() => {
        ...
        void ensureEcho()
            .then((echo) => {
                ...
                const channel = echo.channel('upgrades');
    ...
    const visibleItems = tab === 'top'
        ? items.filter((i) => TOP_RARITIES.includes(i.rarity))
        : items;
