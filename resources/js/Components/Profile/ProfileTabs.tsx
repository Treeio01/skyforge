                    ...
                    <div className="flex gap-1">
                        {selectedSkins.size > 0 && (
                            <button
                                ...
                                className="p-3.5 flex gap-1 items-center border border-[#4E89FF]/30 rounded-[12px] cursor-pointer hover:bg-[#4E89FF]/10 transition-colors"
                            >
                                <SellIcon fill="#4E89FF" />
                                <span className="text-[#4E89FF] font-sf-display text-[13px] font-light leading-[120%] hidden 550:inline">
                        ...
                        <button
                            onClick={onSellAll}
                            className="p-3.5 flex gap-1 items-center border border-[#0A0E17] rounded-[12px] cursor-pointer hover:bg-white/5 transition-colors"
                        >
                            <SellIcon fill="#3C4962" />
                            <span className="text-[#3C4962] font-sf-display text-[13px] font-light leading-[120%] hidden 550:inline">
                                Продать все
                            </span>
                        </button>
                    </div>
...
function UpgradeHistoryCard({ upgrade: u }: { upgrade: UpgradeEntry }) {
    ...
    return (
        <div className={`flex p-2.5 gap-[5px] bg-[#090C13] rounded-[12px] ${u.result === 'lose' ? 'opacity-50' : ''}`}>
            <div className="flex flex-col gap-[5px]">
                ...
                <div className="flex justify-center p-2 bg-[#0B0F18] rounded-[8px]">
                    <span className="text-[#3C4962] font-sf-display text-[11px] leading-[120%] whitespace-nowrap">
...
function TabButton({ active, onClick, icon, label }: { active: boolean; onClick: () => void; icon: React.ReactNode; label: string }) {
    return (
        <button
            onClick={onClick}
            className={`p-3.5 flex gap-1 items-center rounded-[12px] cursor-pointer transition-colors ${active ? 'bg-[#0A0E17]' : 'hover:bg-[#0A0E17]/50'}`}
        >
            {icon}
            <span className={`font-sf-display text-[13px] leading-[120%] hidden 550:inline ${active ? 'text-[#4E89FF]' : 'text-[#3C4962]'}`}>
                {label}
            </span>
        </button>
    );
...
function InventoryTabIcon({ active }: { active: boolean }) {
    const fill = active ? '#4E89FF' : '#3C4962';
    return (
