...
### Шаг 1.14: Установить spatie/laravel-activitylog
```bash
...
### Шаг 1.15: Создать все Eloquent модели (12 штук)
Каждая модель с:
- `$fillable` / `$guarded`
...
Модели:
1. **User** — модифицировать существующую. Traits: `SoftDeletes`, `LogsActivity`. Relations: hasMany(Transaction, Deposit, Withdrawal, Upgrade, PromoCodeUsage), hasOne(ProvablyFairSeed, active)
2. **Skin** — trait: `SoftDeletes`. Relations: hasMany(SkinPrice, UpgradeItem)
...
### Шаг 1.17: Создать SettingsSeeder
Дефолтные значения:
- `house_edge` = 5.00
...
### Шаг 1.18: Запустить миграции и проверить
```bash
php artisan migrate
...
3. **Fetch fresh target skin price from DB** (не из фронта! цена могла измениться)
4. Calculate total bet (skin prices at DB values + balance amount)
5. **Проверить: если цена изменилась > 10% от того что видел юзер → abort с новой ценой**
6. Calculate chance (CalculateChanceAction)
7. Get active seed pair, increment nonce
8. Generate roll (GenerateRollAction)
9. Determine result: `roll < chance/100` → WIN
10. Debit balance (если balance portion)
11. If WIN: credit target skin price to balance
12. Create Upgrade + UpgradeItems records (с snapshot цен)
13. Create Transaction(s)
14. Fire UpgradeCompleted event
15. Return UpgradeResultDTO

> **Важно**: Все цены берутся из БД внутри транзакции, а не принимаются от клиента. Фронтенд передаёт только ID скинов.
